...
## Names of the places that must be visited
@export var places:Array[String]


## Subscribes to [signal ExplorationChannel.region_entered] events.
## Overrides[method Quest._accept].
func _accept():
	ExplorationChannel.region_entered.connect(_on_region_entered)
	super._accept()


## Unsubscribes from [signal ExplorationChannel.region_entered] events.
## Overrides [method Quest._complete].
func _complete():
	ExplorationChannel.region_entered.disconnect(_on_region_entered)
	super._complete()


## While this quest is active, complete the quest when all required regions have been entered.
